...
if __name__ == "__main__":
    ...
    data_list.sort()
    for input_idx, cls_dir in enumerate(data_list):
        os.makedirs(os.path.join(args.output_dir, cls_dir), exist_ok=True)
        print("Running... : {}/{}".format(input_idx, len(data_list)), end='\r')
        for input_name in os.listdir(os.path.join(args.input_dir, cls_dir)):
            input_path = os.path.join(args.input_dir, cls_dir, input_name)
