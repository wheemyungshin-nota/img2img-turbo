...
def build_transform(image_prep):
    ...
    elif image_prep == "sharp_vary_input_s256":
        T = transforms.Compose([
            transforms.ColorJitter(brightness=0.2, contrast=0.2, hue=0.1),
    ...
    def __getitem__(self, idx):
        ...
        if self.split == 'train':
            if random.random() < 0.5:
                input_img = input_img.transpose(PIL.Image.FLIP_LEFT_RIGHT)
                output_img = output_img.transpose(PIL.Image.FLIP_LEFT_RIGHT)
                input_img.save(img_name+'transpose-input.jpg')
                output_img.save(img_name+'transpose-output.jpg')
